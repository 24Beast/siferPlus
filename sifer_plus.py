...
class SiferPlus(nn.Module):
    ...
    def initOptimParams(self, optim_params):
        ...
        self.forget_optim = torch.optim.SGD(self.pre_model.parameters(), lr=self.forget_lr)
    ...
    def initTrainParams(self, train_params):
        try:
            ...
            raise ValueError(f"NumEpochs requires Integer Input! Current Val :{num_epochs}")
    ...
    def train(self, train_loader, train_params):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for i, data in enumerate(train_loader, 1):
                ...
                print(f"\rIteration {i}/{num_batches}: {running_main_loss/i=}, {running_aux_loss/i=}, {running_target_loss/i=}", end="",)
...
if __name__ == "__main__":
    ...
    data_transform = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224), transforms.PILToTensor()])

    models = {"PreModel": PreModel(), "MainModel": MainModel(), "AuxModel": AuxModel()}
    train_data = CelebATarget("./data/", transform = data_transform, split = "train")
