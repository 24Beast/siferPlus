...
class AuxModel(nn.Module):
    def __init__(self):
        ...
        self.layer_1 = nn.Sequential(
            *list(resnet_base_model.children())[5]
        )  #! Take only the second convolutional layer
        self.layer_2 = nn.AdaptiveAvgPool2d(
            (1, 1)
        )  #! Take the average of the output of the second convolutional layer
        self.layer_3 = nn.Flatten()  #! Flatten the output of the average pooling layer

        self.layer_4 = nn.Linear(
            128, 4
        )  #! Output layer for the auxiliary model with 4 classes
        self.layer_5 = nn.Softmax()

        self.layer_6 = nn.Linear(
            128, 1
        )  #! Output layer for the auxiliary model with 1 target
...
class MainModel(nn.Module):
    def __init__(self):
        ...
        self.layer_1 = nn.Sequential(
            *list(resnet_base_model.children())[5]
        )  #! Take the second convolutional layer
        self.layer_2 = nn.Sequential(
            *list(resnet_base_model.children())[6]
        )  #! Take the third convolutional layer
        self.layer_3 = nn.Sequential(
            *list(resnet_base_model.children())[7]
        )  #! Take the fourth convolutional layer
        self.layer_4 = nn.AdaptiveAvgPool2d((1, 1))
        self.layer_5 = nn.Flatten()

        self.layer_6 = nn.Linear(
            512, 4
        )  #! Output layer for the main model with 256 neurons
